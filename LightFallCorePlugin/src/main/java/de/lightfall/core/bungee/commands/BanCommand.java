...
public class BanCommand extends BaseCommand {
    ...
    public void onBan(BungeeCloudUser sender, @Single String player, @Optional String reason) {
        ...
        BridgePlayerManager.getInstance().getOfflinePlayerAsync(player).onComplete((listITask, iCloudOfflinePlayers) -> {
            ...
            this.plugin.getUserManager().loadUser(uniqueId).thenAccept(offlineCloudUser -> {
                ...
                issuer.sendInfo(CoreMessageKeys.BANNED_PLAYER_PERMANENTLY, "{0}", iCloudOfflinePlayers.get(0).getName(), "{1}", lReason);
            });
        });
    }
